...
public class StoreScreen extends Group {
    ...
    public StoreScreen(float w, float h) {
        this.w = w;
        this.h = h;

        backGround = new Image(MainEngine.atlas.findRegion("level_list_bg"));
        topBg = new Image(MainEngine.atlas.findRegion("store_label"));
    ...
    public void start(){
        ...
        enabled = true;
        backGround.setSize(w - 10, h / 1.1f);
        backGround.setX(((w - backGround.getWidth()) / 2 + 5));
        backGround.setY(((h - backGround.getHeight()) / 2)  - 20);
        ...
        topBg.setY(backGround.getHeight() - topBg.getHeight() / 2  - 10);
        topBg.setColor(1, 1, 1, 0);
        topBg.addAction(Actions.alpha(1, 0.2f));
        addActor(topBg);


        done.setPosition(((backGround.getX() + (backGround.getWidth()) / 2) - (done.getWidth() / 2)), backGround.getY() + 20);
